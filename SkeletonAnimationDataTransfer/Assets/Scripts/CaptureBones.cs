...
{
    public int ParentID;
    public int EnumerationValue; //在HumanBodyBones枚举类型中的枚举值
    public Vector3 Offset;
}

public class CaptureBones : MonoBehaviour
{
    ...
    private void initBoneList()
    {
        ...
        for (int i = 1; i < m_BoneDefinition.Count; i++)
        {
            ...
            m_BoneDefinition[i].ParentID = fetchIndexOfNearestAncestor(i);
        }
    ...
    private int fetchIndexOfNearestAncestor(int vCurrentIndex)
    {
        Transform Current = m_BoneTransformList[vCurrentIndex];
        int ParentInstanceID = Current.parent.GetInstanceID();
        ...
        } while (ParentIndex == -1 && ParentInstanceID != RootInstanceID);
    ...
    private void writeBoneListToFile()
    {
        for (int i = 0; i < m_BoneDefinition.Count; i++)
        {
            ...
            string boneStr = bone.ParentID + "," + bone.EnumerationValue + ","
                + bone.Offset.x.ToString("f10") + "," + bone.Offset.y.ToString("f10") + "," + bone.Offset.z.ToString("f10");
